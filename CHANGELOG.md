...
**The sea.** WorldCover has no open-sea class: sea, lagoons, rivers and lakes are all class 80, while this pipeline identifies the sea everywhere as class 200 (sea_mask, both land masks, the coastal weir's ocean mask, zsini, the water-level boundary). 200 is therefore DERIVED -- the sea is the water you reach swimming in from outside the domain, and the rivers are where that stops:
1. barrier = the basin's own clipped river network (rule 06) buffered to each reach's SWORD width, minus anything LC100 calls sea (SWORD lines run past the shoreline at the mouths);
2. sea = water components MINUS that barrier which touch the domain window's border AND hold at least one LC100 open-sea pixel (the guard against inland lakes/upstream rivers that happen to reach the border: on the Mississippi 44 bodies touch it, 3 qualify);
3. plus WorldCover nodata where LC100 says sea -- the open ocean beyond WorldCover's tile footprint, 11,177 km2 in the Mississippi's own domain.

So LC100 remains an input in both modes. Results: Ebro sea 859 km2 (LC100 itself: 842), water left as class 80 down to 26 km2, lagoons still 80, the channel still 80 above its mouth; Mississippi sea 41,766 km2 with the channel blocked. Large sea-connected bays and lakes ARE sea, Lake Pontchartrain included.

A first version instead grew the sea from LC100's own sea cells and capped it at `sea_fringe_m`. That cap cut through physically continuous water, leaving nearshore sea classified as inland water: 47 km2 on the Ebro and 5,781 km2 on the Mississippi, where no distance separates sea from river and marsh (at 3.2 km it still left 7,407 km2 and began leaking up-river). Replaced, and `landuse.sea_fringe_m` with it (now `river_barrier_width_factor`, `river_barrier_min_width_m`).

**Rule order.** Rule 06 is split into `get_river_network` (the clip) and `plot_river_network` (its map), and the clip now runs BEFORE 02b: the clipped network is 02b's barrier, while the plot draws over rule 03's land mask and 03 reads 02b's output. One rule for both would be a cycle; the order is now 02 -> 06 clip -> 02b -> 03 -> 06 plot. Rule 02b uses this per-basin network, never the raw global SWORD.

**Upscaling.** Classes are categorical, so `landuse.tif` (05b) and `landuse_on_grid.tif` (09b) are now resampled by **mode** instead of nearest-neighbour, which kept one arbitrary source pixel per cell (harmless while the source was coarser than the grid, wrong once it is 10 m). Manning's n is continuous, so `roughness.tif` (05c) and `roughness_on_grid.tif` (09b) are **area-averaged from the source at its own resolution** (`landuse.roughness_aggregation`: `area_mean`, default, or `conveyance` = harmonic mean, the equal-depth composite n hydromt's subgrid tables use) rather than reclassified from the already-upscaled class raster -- which handed a cell that is 60% water and 40% built-up water's own 0.02. On basin 2433835's 70 m grid, 30% of cells hold more than one class and 12.9% of all cells change by more than 0.01 in n.

**Roughness resolution.** `roughness.tif` (05c) is now written on the elevation grid SUBDIVIDED to about the source's own pixel size -- ~10 m under WorldCover, the unchanged ~31 m under LC100 (whose 100 m pixels carry no finer detail). hydromt samples exactly this raster per SUBGRID pixel (7 m at `nr_subgrid_pixels: 10`) and averages it by conveyance itself, so its 100 samples per model cell now see ~49 distinct values instead of ~5. Staying an exact subdivision of the elevation grid keeps CRS and alignment with elevation/subgrid. `roughness_on_grid.tif` (09b) stays exactly on the model grid -- it IS the model's own `manning` field.

**Memory.** At 10 m a delta-sized window is 2.0 GIGApixels for the Mississippi (25M for the Ebro), and connected components on that would alone need an 8.2 GB int32 label array -- more than this machine has. Nothing in `src/landuse.py` holds a full fine window: the sea derivation (connected components + the rasterized river barrier) runs on a coarsened working grid capped at 60M px and is applied back by intersecting with the FINE water mask (so the 10 m coastline survives; only the connectivity geometry is resolved coarsely -- factor 6, i.e. a 55 m working grid, on the Mississippi), and every raster written -- 02b's source, 05b's mode-warp, 05c/09b's roughness -- is streamed in row blocks capped at 20M px. Peak memory is set by those caps, not by the delta's size. Measured on the Mississippi (58,197 x 35,077 px): rule 02b 93 s and a 60 MB file at +11 MB peak RSS, 05b's mode-warp onto the 31 m grid 65 s at +1 MB, 05c's 10.31 m roughness (44,766 x 32,106 px) ~10 min and a 396 MB file, with process RSS ending at 46 MB. On the Ebro the streamed result reproduces the earlier in-memory one: identical mean n (0.0411) on the 31 m grid, and the 0.35% of 05b cells that differ are 98.7% class-boundary cells (mode tie-breaking), scattered, not a shift. The source window is now snapped to whole source pixels, so it is a pure window read rather than a 0.07 px-shifted one.

**Averaging only when downsampling** (fixed the same day): `write_roughness_raster` picks `Resampling.average` only where the source is genuinely finer than the target, and nearest otherwise. Area-averaging LC100's 100 m onto a 31 m grid blended neighbouring classes into intermediate n along every class boundary -- 58,859 distinct values where the source holds 7, i.e. invented sub-pixel detail. Nearest there reproduces the old class-value reclassification exactly, so the `copernicus_lc100` path is unchanged; WorldCover still averages.

**Figure writes survive a transient file lock** (a full rule 10 run died on this): on Windows, an existing PNG that another process holds open for a moment -- an antivirus scan, an image preview, Explorer's thumbnailer -- fails the write with `OSError: [Errno 22] Invalid argument`, not the PermissionError one would expect. It killed `modelled_depth_estimation` at its very LAST figure, after all three SFINCS calibration rounds had finished, and Snakemake then deleted the rule's outputs; reproduced afterwards, where a plain `open(path, "w+b")` failed and `savefig` to the same path succeeded seconds later. New `plots.save_figure` retries 4 times (1/2/3/4 s) and then raises naming the likely cause; every figure write in `src/` goes through it (`plots._save` and the three direct calls, `extreme_values`' three, `attribution_plot`'s one). The `savefig` calls inside `workflow/scripts/*.py` are deliberately NOT touched -- editing them would trigger a rerun of those rules -- so they remain exposed until their next edit.

**Plot fixes** (found by the first real WorldCover run, rules 05c/09b): `plot_roughness` built one colour per distinct value, which is right for a raster reclassified from ~10 classes but overflowed matplotlib's `BoundaryNorm` int16 index at 70,268 values (`OverflowError`), and its per-value loop would have been hopelessly slow. It now switches to a continuous 2nd-98th-percentile scale above `_ROUGHNESS_MAX_DISCRETE_CLASSES` (24) and keeps the labelled discrete colorbar below it. `plot_landuse` gained WorldCover's classes 10 (tree cover) and 95 (mangroves) in `_LC_NAMES`/`_LC_COLORS` -- they were drawn unnamed in default grey -- and its title now names whichever source produced the raster, read from the `landuse_source` tag that rule 02b writes and `warp_landuse_to_grid` carries along, instead of hardcoding "Copernicus LC100, 2019".

**Rule 03** vectorizes the same per-basin source, but decimates it by mode to at most 4M px first (`raster.vectorize_land_from_landuse`'s new `max_pixels`): these polygons are a figure background only, and tracing a 10 m window raw means 26M px on the Ebro (2.0 G on the Mississippi) and enormous speckle for no visible gain. Ebro: 0.2 s, 67 polygons, 835 km2 against the source's own 843.

**Also**: `tools/build_esa_worldcover_vrt.py` indexes the downloaded 3x3 degree tiles into the VRT mosaic the catalogue points at (`--check-deltas` reports the tiles each delta polygon still lacks; all 8 are covered); catalogue entry `land_use_esa_worldcover`; `lu_to_roughness_lookup.csv` gained class 10 (tree cover, n=0.12), WorldCover's replacement for LC100's 111-126; `OCEAN_WETLAND_CLASSES` gained 95 (mangroves, intertidal by definition) and moved, with `SEA_CODE`, into the new `src/landuse.py` (re-exported by `src.protection_weir` under its old names); `src.raster.build_roughness_raster` removed as superseded.

Not changed: rule 04's plot background still vectorizes the raw LC100 source over its own wider window, keeping that rule independent of the model-domain chain (and WorldCover, without a sea class, cannot be vectorized as "not sea" directly). Verified on basin 2433835 for both sources: sea extents agree on the source, elevation and model grids; `snakemake -n` resolves with either setting.

# 2026-09-14 Fixed and tested adaptation code, and added more adaptation measures

**Added**: `nbs_land_reclamation` (coastline extension via seaward land reclamation) and `water_retention` (DEM-lowering detention basin + containing weir, sized against a fixed `baseline_excess_volume` from the new attribution mask) preprocessing measures, previously left as follow-up. `urban_raising` (pre- and post-processing) replaces `dike_ring` for the "accommodate" archetype: raises the DEM (pre) or subtracts depth (post) at every flooded urban cell by a fixed additive amount, delta-wide rather than scoped to one hand-drawn ring -- directly comparable to the other delta-wide strategies, and the two variants are designed to match (raising the bed by X m produces the same residual depth as subtracting X m from flood depth, for the same forcing).

**Fixed**: `dike_ring` was silently broken -- `selected_measures` referenced `apply_dike_ring`, which was commented out, raising `NameError` on import; replaced by `urban_raising` instead of repairing it, since a ring around one hand-drawn zone was never comparable to the other delta-wide strategies anyway.

**CHanged**: `discharge_multiplier` (river discharge scaling) moved from one global `config.yml` setting to per-scenario in `scenarios.yml` (`scenario_params()`, `00_common.smk`) -- lets e.g. `river_500`/`compound_500` be amplified to reach genuinely flood-inducing discharge without also scaling `coast_500`'s own small RP=2 river component. This helped not re-run the whole pipeline when I switched between different flood events (and forgot to change the config.yml file).

**Fixed**: `water_retention`'s excavated pit read as pre-filled at t=0 instead of filling only as flood water arrived. Root cause: with spin-up enabled, every run is seeded from a shared, basin-level restart file whose water level (`zs`) was computed on the *original* terrain -- lowering the bed left `zs` untouched, so the same absolute water level landed on a much deeper pit. New module `src/sfincs_restart.py` reads/patches/writes SFINCS's (previously never parsed anywhere in this codebase) binary `.rst` format -- reverse-engineered and empirically validated against basin 2433835 (byte-exact round-trip; `zs` cross-checked via `sfincs.ind`'s own active-cell ordering against `mod.grid.mask`, zero mismatches across all 263,907 cells). `apply_water_retention` now writes a patched copy with `zs` shifted by the change in each coarse cell's own bed reference, preserving whatever relative depth existed before excavation (dry stays dry, already-wet stays at the same relative depth). That shift is computed as `min(dep_new) - min(dep)` per coarse cell (matching hydromt_sfincs's own subgrid table builder, which keys each cell's bed reference on `elevation.min()`) -- an initial version using the MEAN of the fine-resolution elevation change instead under-shifted boundary cells (confirmed: 690/4,657 cells off by >0.5 m, worst case off by nearly the full 16 m excavation on a real run), which spuriously inflated both the pit's own apparent capacity and flooding elsewhere in the domain. `18a_adapt_pre.smk`/`.py` updated so every strategy gets its own local restart (patched for `water_retention`, copied through unchanged otherwise) instead of sharing the basin-level one directly.

Verified: both new/changed modules import cleanly; `snakemake -n` dry runs resolve for representative strategies with and without `water_retention`/`urban_raising`; the restart patch validated end-to-end against the real `NBS_protect_open_1` retention zone geometry (4,657 coarse cells, all correctly shifted).

# 2026-09-11d: dead-code cleanup (- JS)

Following the re-run of `docs/dead_code_audit.md` (now also covering `tests/` and `tools/`).

**Removed** (no call site anywhere):
- `src/raster.py`: `resample_to_utm_array`, `load_raster_to_utm_array`, `clip_raster`.
- `src/river_depth_calibration.py`: `place_reach_midpoint_observations`, `sample_model_bed_at_points` (and the now-unused `Point` import).
- `src/extreme_values.py`: `analyse_cell_gev_only`, `gpd_return_value` (the pipeline uses `gpd_return_value_table`; the `pot_scale` attribute text in `river_forcing.nc` now points there), `EVAResult.as_flat_dict` (and the `asdict` import), and the unused "reserved for future use" locals `_fit_method` / `_ci_alpha` (the config keys are still read by `analyse_cell`).
- `src/protection_levels.py`: unused `_HAZARDS`.
- `scripts/10_depth_estimation_modelled.py`: unused `region_utm`.
- **`{basin}_intersecting_basins.gpkg`**: rule 02 wrote a second copy of the delta polygon whose only reader was rule 08, passing it to an unused `river_basins` parameter of `plots.plot_cleaned_network` (its overlay code was commented out). The output, the rule-08 input, the parameter, the commented block and `domain_bbox.json`'s hard-coded `n_intersecting_basins: 0` are gone.
- 17 orphaned `.pyc` caches whose sources no longer exist (git-ignored, local only).
- **Broken test scripts** (read files, folders or config keys the pipeline no longer has -- `_river_network_processed.gpkg`, `_river_network_estuarine.gpkg`, `landuse_corrected.tif`, `zsini_coastal_dry.tif`, the old `results/{basin}/sfincs/` layout, `sfincs.grid.resolution`, `sfincs.event`, `river_processing.conditioning`): `tests/test_bank_elevation_check.py`, `test_bank_elevation_check_sfincs.py`, `test_discharge_sensitivity.py`, `test_zsini_sensitivity.py`, `test_discharge_return_period_response.py`, `test_subgrid_river_depth_profile.py`, `diagnose_landuse_correction.py`, `plot_main_grid.py`, `animate_water_level.py` (the pipeline's own `02_flood_animation.mp4` covers it). With them `src/river_network.py`'s `trace_seed_mainstem_paths` and `sample_dem_near_river` lost their only user and were removed too (plus five imports only they used).

**Fixed**:
- `plots.plot_hydraulic_relations_with_estuarine` promised to show `L_e_m` but ignored it: the estuary length is now written as an annotation in the correlation panel (the panel plots depth vs depth, so there is no distance axis for a vertical line).
- `plots._OVERLAY_LEGEND_HANDLES`: `Patch(color=..., edgecolor=...)` emitted a matplotlib UserWarning on every import of `src.plots` -> `facecolor=`.
- Rule 05a docstring still said GEBCO is re-referenced by *subtracting* the MDT (now "adding", matching 2026-09-10's sign fix).

**Added**: `05a_elevation_datum_correction.png` carries a note below the map that FathomDEM and the land-use map behind it (grey) are not fully aligned, and that GEBCO is used where FathomDEM has no coverage (new optional `note` argument of `plots.plot_datum_correction_delta`).

**Kept, as decided**: the adaptation code (the unimplemented `storm_surge_barrier` measure and its commented-out stub, the uniform-signature parameters, `18a`'s unused `skeleton_inp_path`), `workflow/archive/` and `tests/archive/`. Rerun impact: the rule 02 / 08 edits change those rules' code, so Snakemake reruns from rule 02 (outputs are unchanged apart from the dropped file/JSON field).

# 2026-09-11c: land masks from the land use on the model grid; last OSM traces removed (- JS)

No OSM data had been read since 2026-08-06, but the naming hid that: `{basin}_land_polygons.gpkg` (rule 03) is the NATIVE Copernicus land use (!= 200) vectorized, yet plot functions called it `osm_land_path`, and it was also used for the models' water-level boundary (`exclude_polygon` in rules 10/13) although the model grid classifies land/sea with `landuse_on_grid.tif` -- a different geometry (2433835: 2 of 783 boundary cells on land use != 200). Rule 11b still downloaded real OSM web tiles; rules 13 (skeleton diagnostics) and 18c (attribution map) satellite tiles.

**Changed**:
- **Boundary**: rules 10 and 13 create the water-level boundary without polygons, then `src.raster.restrict_waterlevel_boundary_to_sea` returns every boundary cell whose `landuse_on_grid` != 200 to a normal active cell (2433835: 783 -> 781). **Bug fixed the same day**: this first called `create_boundary(btype="waterlevel", reset_bounds=True)` -- with no polygon or elevation filter hydromt's `reset_bounds=True` only resets and returns, so no boundary cell was set and rule 10 failed at `create_boundary_points_from_mask` ("The mask should have values greater than 1"). Now `reset_bounds=False` (the mask is fresh from `create_active` anyway; 2433835: 1455 edge cells -> 780 on open sea, 21 boundary points), and both rules raise a clear error if no sea boundary cell remains.
- **Grid-aligned land mask**: rule 09b (`grid_align_landuse`) writes `{basin}_land_mask_on_grid.gpkg` (`src.raster.vectorize_land_mask_on_grid`: land use != 200 traced from `landuse_on_grid.tif`, vertices exactly on cell corners, WGS84) -- the background of every figure from 09b on (rules 09b, 10 both variants, 11 upstream check, 13, 14, 15, 16, 18a run, 18b, 18c, `tools/replot_event_figures.py`, test plot scripts). Figures made before the SFINCS grid exists (rules 04-09, 11b) keep rule 03's native mask.
- **Web basemaps removed**: 11b's `cimgt.OSM()` tiles -> rule 03's mask; rule 13's four `plot_basemap(bmap="sat")` figures and 18c's attribution map -> the grid mask via new `plots.add_land_background_to_geoaxes`. No internet needed for any figure.
- **Cleanup**: `osm_land_path` -> `land_polygons_path` (plots.py, extreme_values.py, all call sites); the `water_bodies_path` overlay (white patches of land use 200 from a third raster, `landuse.tif`) removed everywhere -- both masks already exclude 200, so it only added a misaligned second outline -- and with it the `landuse.tif` inputs rules 06, 11, 15, 16 and 18a-run took only for that; stale "from OSM land polygons" comment in `src/raster.py`; stale OSM entries in `Reference_memory.txt`. Also fixed `tests/animate_water_level.py` passing `landuse.tif` as its sea mask (never equals 1, so nothing was masked).

Verified: land mask area == land cell count x cell area (664.409 km2 both) with every vertex on a cell corner; boundary helper on the real skeleton mask; a `plot_basemap` figure with the land background renders; code-health check clean; `snakemake -n` resolves. Not changed: `tests/test_zsini_sensitivity.py` / `tests/test_discharge_sensitivity.py` still use the old `exclude_polygon` build -- both were already broken (config keys/files that no longer exist).

# 2026-09-11b: coastal flooding below the protection level -- spin-up startup wave and coastal crest (- JS)

`coast_10` (surge RP 10, river RP 2; protection RP 41 coastal / 51 riverine) showed ~60 km2 of flooding behind the dike. Diagnosed on basin 2433835 (MDT and weirs were fine -- SFINCS reads the weir in spin-up and event; the 290 weir segments SFINCS drops are links whose ground is already above the crest): sea water came OVER the coastal dike at two moments.

1. **Spin-up startup wave.** The sea starts at the calm-sea level (0.0 m) but the spin-up boundary jumped straight to the RP-1 level (0.30 m). The step travels in, shoals and reflects: up to +1.1 m at the 0.40 m coastal crest, up to 650 coastal segments overtopped for ~2 h. The water fills protected lagoons/wetlands (beds ~-1 m), can't drain back over the crest, and the restart carries it into EVERY event run -- 15,440 of coast_10's 17,404 flooded land cells were already wet when the event started, at levels below sea level. A second mismatch: the spin-up ENDED at RP 1 (0.30 m) while every event's boundary lead-in starts at the calm-sea level (0.00 m), so the boundary dropped 0.30 m the instant the event started from the restart. **Fixed**: the spin-up sea is now held at each station's calm-sea level (`calm_sea_levels`, the event lead-in's own level; the river stays at RP 1), ramped from the sea's starting level (zsini's rounded basin mean -- normally identical) over `sfincs.spinup.boundary_ramp_hours` (6 h). Verified by rerunning the 24 h spin-up that way: coastal segments overtopped 602 -> 0, protected cells wet at the end 10,622 (~52 km2) -> 6 (the one small undiked lagoon patch); spin-up end level == coast_10's lead-in at every station. Rule 18c's "spin-up permanent water" class now means wet under an RP-1 river with a calm sea.
2. **Coastal crest vs simulated coastal water level.** The crest was the offshore COAST-RP level at RP 41, but SFINCS's sea level at the dike rises well above the boundary level (coast_10: up to ~0.8 m under a 0.4 m boundary peak; 1,491 of 2,243 coastal segments overtopped at the peak hour). Worsened at the Ebro by a very flat storm-tide curve -- 0.264 m (RP 1) to 0.449 m (RP 1000); RP 10 vs RP 41 only 3.7 cm apart, which the 0.1 m round-up erases (both 0.40 m). **Fixed**: rule 10 forces rounds 1 and 2 with a storm tide at the coastal protection RP (FLOPROS; per station, linear in RP between COAST-RP's tabulated RPs like the crest itself -- new `src.surge.storm_tide_at_rp_interpolated`; production's half-cosine over `surge.period_hr`, starting after the discharge ramp, rising from `baseline_m`), at the production surge stations. Every coastal edge's crest then comes from the simulated level at the dike (per-edge rule, floored at the FLOPROS crest), and round 2 verifies it. River and surge at their protection RPs together is conservative near the mouth. Round 0 (excavation) keeps the steady calm-sea boundary. Raises ValueError if `calibration_days` is shorter than discharge ramp + surge period.

Kept, as decided: the 0.1 m round-up on the forcing (storm tide and calm sea) as well as on the crests. Not changed: the flood metrics still count the river channel (coast_10: 11.4 km2, the 15 m "max depth") and permanent lagoons as flooded land.

**Also** in `config/scenarios.yml`: `coast_10` (surge RP 10 / river RP 2, the scenario this diagnosis ran on), plus `river_50`, `coast_50` and `compound_50` alongside the existing RP-500 set.

# 2026-09-11: inundation-depth maps use a fixed 5 m colour-bar maximum (- JS)

`plot_inundation_check` (01_inundation_ratio), `plot_max_inundation_map` (spin-up / calibration max inundation) and the depth frames of `animate_flood_progression` used to stretch their colour bar to each map's own 99th percentile, so the same colour meant a different depth in every figure. They now share `plots._INUNDATION_DEPTH_CBAR_MAX_M = 5.0` (deeper cells saturate, colour bars keep `extend="max"`). Water-level animations are unchanged (true min/max).

# 2026-09-10e: weir discharge coefficient 1.0 -> 0.6, and the documented overtopping formula corrected (- JS)

`river_depth_modelling.weir_par1` is now **0.6**, SFINCS's own recommended value for `par1` (also hydromt-sfincs' default), instead of 1.0. It is the discharge coefficient Cd of every weir this pipeline builds -- the coastal protection dike and the river-confinement walls -- so it scales overtopping discharge directly: the previous 1.0 passed ~1.7x the flow of the new value for the same exceedance.

The config comment describing the formula was also wrong, and is now SFINCS's actual weir law. Per metre of weir, with h1/h2 the water level above the crest on the high/low side: free flow `q = Cd * 1.7049 * h1^1.5` where 1.7049 = (2/3)^1.5 * sqrt(g) -- the factor the old comment (`Q = Cd*L*head^1.5`) left out entirely; submerged flow `q = Cd * h2 * sqrt(2g (h1 - h2))` once h2 > 2/3 h1; and NO flow at all while both sides sit below the crest, which is what makes the crest a real threshold rather than a resistance. `tests/plot_overtopping_volume.py` reproduced the old, incomplete formula and now matches.

# 2026-09-10d: option to leave ocean-linked wetlands outside the coastal dike (- JS)

**Added**: `river_depth_modelling.unprotected_ocean_wetlands` (default `false`). When `true`, `build_coastal_protection_weir` adds every wetland/lagoon patch (Copernicus LC100 80 = permanent water bodies/tidal flats, 90 = herbaceous wetland; `OCEAN_WETLAND_CLASSES`) that shares a cell edge with the open sea (`ocean_linked_wetland_mask`) to the seaward side, so the coastal dike runs along the patch's landward edge and the patch floods freely. 4-connectivity on purpose (SFINCS only exchanges water across shared edges; a corner-touching patch could never fill from the sea). The cells stay land otherwise -- not sea in `zsini_sea_cells`, counted in flood metrics. Applied in all three calibration rounds and the exported weir, so switching it reruns rule 10 and everything downstream. Basin 2433835 (geometry-only check on the real grid): 8,986 wetland cells (~44 km2) move outside the dike, plus 325 cells of small (<30-cell) land islands inside them that the existing `min_component_cells` rule dissolves; weir 280 -> 287 km. `tests/check_crest_calibration.py` now reports wetland-edge segments as their own category. New rule-10 output `visuals/10_calibration/dike_positions_wetland_switch.png` (`plots.plot_wetland_dike_positions`): both dike traces overlaid on the land-use classes, every run regardless of the setting (the other setting is traced for the figure only) -- grey = same position, red = OFF only, blue = ON only, zoom on the largest ocean-linked wetland patch. Also fixed rule 10's plot titles, which showed "preprocessing_inputs" instead of the basin ID (`calib_root.parent.name` -> the `basin_id` wildcard).

# 2026-09-10c: fixed the MDT sign -- MSL-referenced GEBCO and COAST-RP now get +MDT, not -MDT, to reach GOCO06s (- JS)

MDT (HYBRID-CNES-CLS22) is the mean sea surface's height ABOVE the GOCO06s geoid (CF `standard_name: mean_dynamic_topography`; the file itself shows +0.60 m in the Sargasso Sea, +1.12 m in the western Pacific warm pool, -1.46 m in the Southern Ocean), so H_GOCO06s = H_MSL + MDT -- `data_catalogue.yml` already stated the inverse, H_MSL = H_GOCO06s - MDT. Both MSL-referenced inputs were converted with the wrong sign: rule 05a did `gebco -= mdt` and rule 07 `rp_level = rp_level_raw - mdt` (and `coastal_protection_crest_m = mean_prot_raw - mean(mdt)`). The subtraction is the GOCO06s -> MSL direction, carried over from the archived earlier pipeline that converted the DEM itself to local MSL (`workflow/archive/datum_correction/`). Effect: seabed, calm sea, storm-tide boundary and the coastal crest floor all sat 2*MDT off relative to the (correctly EGM2008 -> GOCO06s-shifted) FathomDEM land -- 2433835 (Ebro, MDT ~ -0.05 m): water ~0.10 m too HIGH; deltas in high-MDT regions (W Pacific, MDT up to ~+1 m) had water up to ~2 m too LOW.

**Fixed**: `src.surge.apply_mdt_correction` and 07's own recomputation use `+ mdt`; 05a uses `gebco += mdt`; `coastal_protection_crest_m = mean_prot_raw + mean(mdt)`; `baseline_m`/`station_baseline` follow automatically (derived as rp_level - rp_level_raw). Diagnostic plots/labels (`plot_surge_corrections`, the surge table, the GEBCO delta title), docstrings, `data_catalogue.yml` and `Reference_memory.txt` updated. Verified on 2433835's stored stations: every level shifts by 2*MDT (-0.092 to -0.110 m); rounded calm sea +0.10 -> 0.00 m. Reruns everything from rules 05a (`get_elevation`) and 07 (`get_boundary_forcings`) downstream -- confirmed via `snakemake -n`.

# 2026-09-10b: weir crests and COAST-RP design water levels rounded UP to the next 0.1 m (- JS)

`sfincs.weir` stores crests at 0.1 m precision with round-to-nearest (hydromt_sfincs `write_geoms`, `fmt_z="%.1f"`): on basin 2433835, 17% of segments were written up to 5 cm below their computed crest, i.e. they started overtopping early. **Changed**: new `src.surge.ceil_water_level` rounds up to the next 0.1 m (float-noise-safe: 0.3 stays 0.3). Applied to every production weir crest in `build_coastal_protection_weir` (after the max(water level, coastal floor) + freeboard_m) -- so what SFINCS reads is exactly the computed crest, never lower -- and to every absolute coastal water level, after MDT/SLR (so in model datum): the COAST-RP storm-tide peak (`lookup_storm_tide_at_rp`: ceil(calm level + storm tide)), the per-station calm-sea lead-in (`calm_sea_levels`), and the scalar `baseline_m` (`read_baseline_m`, now THE reader for rule 10's calibration boundary + zsini and rule 13's skeleton zsini, so start level and boundary agree). Feeds the event boundary (`build_design_surge_matrix`, rule 13), the RP=1 spin-up boundary (rule 14) and rule 11's upstream-boundary check. Basin 2433835: baseline_m +0.051 -> +0.10 m (mean sea level up by ~5 cm everywhere). `surge_forcing.nc` itself is unchanged (rounding happens at read/build time), so rule 07 doesn't rerun. `tests/plot_surge_vs_dike_crest.py` now compares against the floor as built (+ freeboard, rounded up).

# 2026-09-10: river depth/crest calibration reduced to three fixed runs -- crest set per weir edge from the confined, excavated water level (- JS)

Rule `modelled_depth_estimation` used to run up to 11 SFINCS rounds: round 0 split the confined, UN-excavated rise between excavation (80%) and a crest (20%), then up to `n_correction_iterations` coupled rounds raised the crest by `min_crest_increment_per_round_m`, snapped it down to zs + a hardcoded 0.5 m and re-verified, with separate coastal-probe and river-boundary-probe corrections and a nearest-anchor/dilation scheme to paint centerline crests onto the bank cells. It only needed to iterate because the crest was guessed from the un-excavated water level.

**Changed**: three fixed runs of the same calibration model. Round 0 -- 1000 m walls, un-excavated: `rivdph = excavation_fraction * max(zs_max - DEM, 0)` per centerline cell (new `excavation_fraction`, set to 0.8 -- the same excavation share the old `weir_crest_fraction: 0.2` implied), then the existing mouth rules (coastal endpoint at natural bathymetry, bed floored upstream of it). Round 1 -- same walls, excavated channel: with no overbank relief its water level is the highest the calibration discharge can reach, so every weir edge's crest is set directly from it: `crest(edge) = max(zsmax(water-side cell), coastal_protection_crest_m) + freeboard_m`. The same rule covers riverbanks, the mouth, discharge-injection cells and the coast near the river plume. Uses SFINCS's own `zsmax` (every timestep), not the hourly `zs` max: on basin 2433835's old round 0 the hourly max sat up to ~0.1 m below `zsmax` at 1% of wet cells. Round 2 -- verification with the real weir: logs any edge whose water side exceeds its crest and writes the crest gap map, but changes nothing.

`build_coastal_protection_weir` now takes `water_side_crest_on_grid` (looked up on each edge's water side) in place of `river_crest_on_grid`/`coastal_crest_on_grid`/`river_crest_dilation_cells`, and returns `crest_surface`/`edge_water_side_mask` in its diagnostics. `config.yml`: `excavation_fraction` added; `weir_crest_fraction`, `n_correction_iterations`, `min_crest_increment_per_round_m`, `river_crest_dilation_cells` removed; `freeboard_m` is now the only margin (the hidden 0.5 m calibration freeboard is gone). **Removed** as dead code: `GridArrays.dilate_values`, `river_burn.build_nearest_weir_crest_regular`/`build_smoothed_weir_crest_regular`/`_smoothed_weir_crest_profiles`, `river_depth_calibration.compute_calibrated_depth`/`compute_calibrated_weir_crest` (replaced by `compute_excavation_depth`).

Verified: `tests/check_code_health.py` clean; per-edge water-side lookup checked on a synthetic grid (channel, ocean, dry-cell fallback, domain-edge closure); the full-field `zsmax` reader's (n, m) mapping checked cell-for-cell against basin 2433835's existing round-0 `sfincs_map.nc`. Not yet run end-to-end -- left to the user to trigger (rerun `modelled_depth_estimation`; old `round3`-`round10` directories from the previous scheme stay on disk until deleted).

# 2026-08-21 Added adaptation effectiveness assessment (KL)
